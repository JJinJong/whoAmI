...
public class ManagerExpertListOk implements Action{
...
//		int total = bDao.getTotal();
		...
		ProfileMap.put("startRow", startRow);
		ProfileMap.put("rowCount", rowCount);
		//시작 인덱스와, 개수를 전달하여 게시글 목록을 가져온 뒤 requestScope에 담아준다.
		req.setAttribute("profileList", epDao.selectAll(ProfileMap));
...
//		req.setAttribute("boardList", boardDTO_list);
		...
		return new ActionForward(false, "/app/management/expert/solo/profileList.jsp");
	}
}
