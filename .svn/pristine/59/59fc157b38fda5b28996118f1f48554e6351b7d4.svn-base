...
function getCommentList() {
	$.ajax({
		url : contextPath+"/expertPage/ExpertCommentListOK.ddy",
		type : "get",
		dataType : "json",
...
//							text += '                            	<div class="action commentDetail" data-index="' + index + '" >보기</div>'
							...
							text += '                    <div class="comment">' + info.worryCommentManagerContent + '</div>'
							text += '                </div>'
							...
							text += '						<textarea data-index="' + index + '" placeholder="따뜻한 관심을 보여주세요" class="contents EditContent" style="height: 36px;">'
									+ info.worryCommentManagerContent + '</textarea>'
							text += '							<button class="btn submit commentUpdateOk" data-index="' + index + '" type="button">'
							text +='									<input data-index="' + index + '" type="hidden" class="worryCommentManagerNumber" name="worryCommentManagerNumber" value="'+ info.worryCommentManagerNumber +'"> '
							text += '							<svg stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 512 512" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg">'
...
//댓글 기능
	...
	$(".commentUpdateOk").each(function(index, item){
		$(item).click(function(){
			...
			const worryCommentManagerNumber=$($(".worryCommentManagerNumber")[index]).val();
			const EditContent=$($(".EditContent")[index]).val();
			console.log(worryCommentManagerNumber);
			if (confirm("정말 수정하시겠습니까 ?")) {
				$.ajax({
					url : contextPath + "/expertPage/ExpertCommentupDateOK.ddy",
					type : 'get',
					data : { "worryCommentManagerNumber" : worryCommentManagerNumber,"worryCommentManagerContent" : EditContent},
					datatype:"JSON",
	...
	$(".commentDelete").each(function(index, item){
		$(item).click(function(){
			...
			if (confirm("정말 삭제하시겠습니까 ?")) {
				$.ajax({
					url : contextPath + "/expertPage/ExpertCommentDeleteOK.ddy",
					type : 'get',
					data : { "worryCommentManagerNumber" : worryCommentManagerNumber},
					datatype:"JSON",
					success : 
						function(res){
						alert('삭제 완료');
	...
	$(".commentDetail").each(function(index, item){
		$(item).click(function(){
			...
			const worryCommentManagerNumber=$($(".worryCommentManagerNumber")[index]).val();
			console.log(worryCommentManagerNumber);
			location.href = contextPath + "/worry/WorryDetailOK.bo?worryCommentManagerNumber="+ worryCommentManagerNumber;
		})
	});
}
